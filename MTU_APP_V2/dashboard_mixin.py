...
class DashboardMixin(AppMixin):
    ...
    def create_overview_page(self):
        ...
        # Card 1 — Current Budget Limit (green)
        self.balance_card_frame = ctk.CTkFrame(
            cards_frame, corner_radius=24, fg_color="#059669", height=120
        )
        self.balance_card_frame.grid(row=0, column=0, sticky="nsew", padx=10)
        self.balance_card_frame.grid_propagate(False)
        ...
        ctk.CTkLabel(
            ...
            font=("Segoe UI", 16),
            text_color="white"
        ).pack(expand=True, anchor="center", pady=(18, 2))
        ...
        self.balance_card.pack(expand=True, anchor="center", pady=(0, 14))

        # Card 2 — Saving Streak (with fire indicator)
        self.streak_card = ctk.CTkFrame(
            cards_frame, corner_radius=24, height=120
        )
        self.streak_card.grid(row=0, column=1, sticky="nsew", padx=10)
        self.streak_card.grid_propagate(False)
        ...
        streak_row = ctk.CTkFrame(self.streak_card, fg_color="transparent")
        streak_row.pack(expand=True, anchor="center", pady=(0, 14))

        self.streak_value = ctk.CTkLabel(
            streak_row,
            text="Current: 0 | Best: 0",
            font=("Segoe UI", 20, "bold"),
            text_color="#F59E0B"
        )
        self.streak_value.pack(side="left", padx=(0, 6))
        ...
        self.expense_card_frame.grid_propagate(False)
        ...
        self.expense_card.pack(expand=True, anchor="center", pady=(0, 14))

        # ── Middle row: Spending Overview + Recent Transactions ─────
        overview_row = ctk.CTkFrame(page, fg_color="transparent", height=330)
        ...
        analytics_frame = ctk.CTkFrame(overview_row, corner_radius=24)
        ...
        self.recent_transactions_frame = ctk.CTkScrollableFrame(
            transaction_frame, height=155, corner_radius=18
        )
        ...
        # ── Bottom row: Goals & Progress + Goal Status (same size) ──
        bottom_frame = ctk.CTkFrame(page, fg_color="transparent")
        bottom_frame.pack(fill="x", padx=20, pady=(0, 18))
        bottom_frame.grid_columnconfigure(0, weight=1, uniform="bottom_cards")
        bottom_frame.grid_columnconfigure(1, weight=1, uniform="bottom_cards")

        # Goals & Progress card
        self.goal_preview = ctk.CTkFrame(
            bottom_frame, corner_radius=24, height=200
        )
        self.goal_preview.grid(row=0, column=0, sticky="nsew", padx=10)
        ...
        self.remaining_budget_label.pack(anchor="w", padx=20, pady=(0, 8))

        self.remaining_budget_bar = ctk.CTkProgressBar(self.goal_preview, height=18)
        self.remaining_budget_bar.pack(fill="x", padx=20, pady=(0, 14))
        self.remaining_budget_bar.set(0)

        # Goal Status card — same height, bigger text
        self.budget_preview = ctk.CTkFrame(
            bottom_frame, corner_radius=24, height=200
        )
        self.budget_preview.grid(row=0, column=1, sticky="nsew", padx=10)
        self.budget_preview.grid_propagate(False)

        ctk.CTkLabel(
            self.budget_preview,
            text="Goal Status",
            font=("Segoe UI", 22, "bold")   # same size as Goals & Progress header
        ).pack(anchor="w", padx=20, pady=(20, 6))
        ...
        self.goal_status_frame.pack(fill="both", expand=True, padx=10, pady=(0, 12))
    ...
    def create_summary_page(self):
        ...
        ctk.CTkLabel(
            header, text="Summary", font=("Segoe UI", 30, "bold")
        ).pack(side="left")
        ...
        self.current_budget_card = ctk.CTkFrame(
            page, corner_radius=24, fg_color="#0F766E", height=140
        )
        self.current_budget_card.pack(fill="x", padx=20, pady=(0, 20))
        ...
        colors = [
            ...
            "#EF4444", "#EC4899", "#22C55E", "#6366F1"
        ]

        for i, category in enumerate(self.categories):
            card = ctk.CTkFrame(page, corner_radius=20)
            ...
            ctk.CTkLabel(
                top, text=category, font=("Segoe UI", 18, "bold")
            ).pack(side="left")

            amount_label = ctk.CTkLabel(
                top, text="₱0 / ₱0", font=("Segoe UI", 14)
            )
            amount_label.pack(side="right")

            pb = ctk.CTkProgressBar(
                card, progress_color=colors[i % len(colors)], height=15
            )
    ...
    def refresh_dashboard(self):
        ...
        self.balance_card.configure(text=f"₱{limit:,.2f}")

        if hasattr(self, "current_budget_amount"):
            self.current_budget_amount.configure(text=f"₱{limit:,.2f}")

        self.expense_card.configure(text=f"₱{remaining:,.2f}")

        if hasattr(self, "remaining_budget_label"):
            self.remaining_budget_label.configure(text=f"Remaining: ₱{remaining:,.2f}")
        if hasattr(self, "remaining_budget_bar"):
            progress = max(0, min(remaining / limit, 1.0)) if limit > 0 else 0
            self.remaining_budget_bar.set(progress)

        # Goal Status card — bigger font for goal names
    ...
    def render_recent_transactions_cards(self):
        if not hasattr(self, "recent_transactions_frame") or not self.current_user \
        ...
        for row in rows:
            ...
            card = ctk.CTkFrame(self.recent_transactions_frame, corner_radius=18)
